...
namespace Nova::Core::Renderer::RHI {
    ...
    bool CompileSlangFileToSpirv(
        ...
        ExtractProgramReflection(linked.get(), input.m_Stage, out.m_Reflection);
        if (out.m_Reflection.m_Sets.empty()) {
            AppendSlangReflectionJsonExcerpt(log, linked.get());
        }
    ...
    RHI_ShaderCompileResult RHI_ShaderCompiler::Compile(const RHI_ShaderCompileInput& input) {
        ...
        const std::string hash = RHI_ShaderCache::ComputeHash(in);

        if (!in.m_SkipCache) {
            RHI_ShaderCompileResult cached{};
            if (RHI_ShaderCache::TryGetMemory(hash, cached)) {
                return cached;
            }

            if (!RHI_ShaderCache::NeedsRecompile(in, hash)) {
                RHI_ShaderCompileResult disk{};
                if (RHI_ShaderCache::LoadDisk(hash, disk)) {
        ...
        if (!in.m_SkipCache) {
            ...
            RHI_ShaderCache::PutMemory(hash, out);
        }
