...
namespace Nova::Core::Renderer::RHI {
	...
	struct NV_API RHI_Mesh {
		...
		RHI_Mesh() = default;
		RHI_Mesh(std::vector<Math::Vertex> vertices, std::vector<uint32_t> indices): m_Vertices(vertices), m_Indices(indices) {}
		virtual ~RHI_Mesh() = default;

		const std::vector<Math::Vertex>& GetVertices() const { return m_Vertices; }
